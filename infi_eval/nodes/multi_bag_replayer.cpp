...
#include "infi_msgs/StartEvaluation.h"
...
public:
	...
	void Initialize( const ros::NodeHandle& ph )
	{
		...
		for( iter = infos.begin(); iter != infos.end(); ++iter )
		{
			...
			playerServer = bh.advertiseService<infi_msgs::StartEvaluation::Request,
			                                   infi_msgs::StartEvaluation::Response>
			                   ( "start_playback",
	...
	bool StartCallback( rosbag::Player& player,
	                    infi_msgs::StartEvaluation::Request& req,
	                    infi_msgs::StartEvaluation::Response& res )
	{
		if( _resetFilter )
		{
